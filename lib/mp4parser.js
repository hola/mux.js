...
var Chunk_parser = function(opt){
    ...
    this.frag_size = opt.frag_size||50;
};
...
Chunk_parser.prototype.parse = function(opt){
    ...
    while (pc)
    {
        ...
        for (i=0; i<this.s_p.length; i++)
        {
            ...
            if (pos>=b_start && pos+sz<=b_end)
            {
                ...
                sample.dts = this.s_info[i].s_time[sn];
                sample.pts = sample.dts+(this.s_info[i].s_ctts[sn]||0);
                this.s_p[i].max_t = Math.floor(sample.dts/this.s_info[i].ts);
                ...
                sample.synced =
                    !(this.s_info[i].s_sync.length||(sn+1)%this.frag_size)||
                    this.s_info[i].s_sync.indexOf(sn+1)>-1;
                if (this.s_info[i].type=='vide' && this.break_on_sync &&
                    sample.synced)
                {
                    opt.stream.flush();
                }
    ...
    for (i=0; i<this.s_p.length; i++)
    {
        ...
        if (this.s_p[i].max_t<max_time)
        {
            max_time = this.s_p[i].max_t;
            new_pos = this.s_info[i].s_off[this.s_p[i].s];
        }
    }
    if (new_pos==b_start)
        new_pos = b_end;
...
Chunk_parser.prototype.seek = function(time, use_ss){
    ...
    function get_frame(target, elm, use_ss){
        var l, r, sn, m, m_time, tt = target;
        var i_ss = use_ss&&elm.s_sync.length;
        var i_soun = elm.type=='soun';
        var scale = elm.ts/90000;
        if (elm.elst&&elm.elst.length)
        {
            var t_dur = 0;
            for (var i=0; i<elm.elst.length; i++)
            {
                if (!elm.elst[i].segment_duration ||
                    tt<t_dur+elm.elst[i].segment_duration)
                {
                    var m_corr = i_soun ?
                        Math.floor(elm.elst[i].media_time/scale) :
                        elm.elst[i].media_time;
                    tt += m_corr-t_dur;
                    break;
                }
                t_dur += elm.elst[i].segment_duration;
            }
        }
        ...
        {
            // no sync -> check back for greater PTS
            m_time = elm.s_time[l]+(elm.s_ctts[l]|0);
            for (sn=l; sn>l-10; sn--)
            {
                if (elm.s_time[sn]+(elm.s_ctts[sn]|0)>m_time)
                    l = sn;
            }
        }
        return i_ss ? elm.s_sync[l] : l;
    }
    if (this.v_idx!==undefined)
    {
        var elm = this.s_info[this.v_idx];
        var v_sn = get_frame(target, elm, use_ss);
        this.s_p[this.v_idx].s = v_sn;
        this.s_p[this.v_idx].max_t = elm.s_time[v_sn]/elm.ts;
        // sync audio track(s) to video
        target = Math.floor(this.s_p[this.v_idx].max_t*90000);
    ...
    this.s_info.forEach(function(elm, idx){
        if (idx==_this.v_idx)
            return;
        var s_sn = get_frame(target, elm, use_ss);
        _this.s_p[idx].s = s_sn;
        _this.s_p[idx].max_t = elm.s_time[s_sn]/elm.ts;
...
Buffer.prototype.push = function(chunk){
    ...
    {
        var _newbuff = new Uint8Array(Math.max(this.pos+c_len,
...
MP4ParserStream.prototype.seek = function(time, use_ssync){
    return this.c_parser.seek(time, use_ssync); };
