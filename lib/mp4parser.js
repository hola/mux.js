...
var bit_reader = function(view, ptr, size){
    var pos = 0, len = size*8;
    this.read = function(count, peek){
        var ret = 0;
        for (var i = 0, p = pos>>3, r = 7-pos%8, c = view.getUint8(p+ptr);
            i<count; i++, r--)
        {
            ret = (ret<<1)|((c>>r)&1);
            if (!r)
...
Box_parser.prototype._parse_esds = function(opt, elm){
    ...
    while (ptr<opt.ptr+opt.size)
    {
        ...
        case 5: // DecoderSpecificInfoTag
            var br = new bit_reader(view, ptr, sz);
            elm.aot = br.read(5);
            if (elm.aot==31)
            ...
            if (elm.aot!=5 && elm.aot!=29 && elm.aot!=36)
            {
                while (br.bits()>=16)
                {
        ...
        for (i=0; i<this.s_p.length; i++)
        {
            ...
            if (pos>=b_start && pos+sz<=b_end)
            {
                ...
                if (sinfo.type=='vide' && (this.break_on_count ?
                    !(sn%this.frag_size) : sn&&sample.synced))
                {
                    opt.stream.flush();
                }
