...
var Box_parser = function(opt){ this.conf_update(opt); };
Box_parser.prototype = {};
Box_parser.prototype.conf_update = function(conf){
    this.force_he_aac_on_low_freq = conf.force_he_aac_on_low_freq;
};
Box_parser.prototype.header = function(opt){
    ...
    if (full_box.includes(opt.type))
    {
        ...
        if (opt.buffer.b_size-opt.ptr<opt.offset)
            return (opt.offset = 0);
        var extra = opt.view.getUint32(opt.ptr+opt.offset-4);
        opt.ver = extra>>>24;
...
Box_parser.prototype._parse_avcc = function(opt, elm){
    elm.avcc = new Uint8Array(opt.size);
    elm.avcc.set(opt.buffer._buff.subarray(opt.ptr, opt.ptr+opt.size));
    var view = opt.view, ptr = opt.ptr;
    var sps = elm.sps = [];
    var pps = elm.pps = [];
...
Box_parser.prototype._parse_esds = function(opt, elm){
    ...
    while (ptr<opt.ptr+opt.size)
    {
        ...
        case 5: // DecoderSpecificInfoTag
            var ext_type, sbr = -1, br = new Bit_reader(view, ptr, sz);
            elm.aot = br.read(5);
            if (elm.aot==31)
            ...
            if (ext_type!=5 && elm.aot!=36)
            {
                ...
                // HE-AAC uses SBR, high freqs are constructed from low freqs
                // there is a factor 2 between frame sample rate and
                // output sample rate
                if (this.force_he_aac_on_low_freq && elm.aot==2 && !elm.dsi &&
                    elm.channel>1 && elm.freq>=6 && sbr==-1)
                {
                    elm.dsi = 5;
                    elm.ext_freq = elm.freq-3;
                }
            }
            ptr += sz;
            break;
...
Box_parser.prototype.stsd = function(opt){
    ...
    for (i=0; i<count; i++)
    {
        ...
        case 'vide':
            ...
            while (skip_boxes.includes(view.getUint32(opt.ptr+4)))
                opt.ptr += view.getUint32(opt.ptr);
            switch (view.getUint32(opt.ptr+4))
            {
...
Chunk_parser.prototype.conf_update = function(conf){
    ...
    this.disable_val_frame_nal_len4 = conf.disable_val_frame_nal_len4;
};
Chunk_parser.prototype.process = function(opt){
    ...
    opt.root.movie_box.track_info.forEach(function(tr){
        ...
        else
        {
            ...
            if ((avcc = dr.avcc))
            {
                event_elm.codec = 'avc1.'+byte_to_hex(avcc.avc_p_i)+
                    byte_to_hex(avcc.prof_compat)+byte_to_hex(avcc.avc_l_i);
                var info = [1, avcc.avc_p_i, avcc.avc_prof_compat,
                    avcc.avc_l_i, 255, avcc.sps.length+224];
                avcc.sps.forEach(function(e){
                    info = info.concat([e.nal.length>>8, e.nal.length&255]);
                    Array.prototype.push.apply(info, e.nal);
                });
                info.push(avcc.pps.length);
                avcc.pps.forEach(function(e){
                    info = info.concat([e.nal.length>>8, e.nal.length&255]);
                    Array.prototype.push.apply(info, e.nal);
                });
                event_elm.s_i = new Uint8Array(info);
                event_elm.avcc = avcc.avcc;
            }
            else if (dr.hvcc)
                event_elm.codec = 'hvc1';
            else if (dr.esds)
                event_elm.codec = 'mp4v';
        }
        ...
        for (i=0; i<this.s_p.length; i++)
        {
            ...
            if (pos>=b_start && pos+sz<=b_end)
            {
                ...
                if (sinfo.type=='vide' && (this.break_on_count ?
                    sn%this.frag_size===0 : sample.synced))
                {
                    opt.stream.flush();
                }
                var avcc = sample.dr&&sample.dr.avcc;
                if (!this.disable_val_frame_nal_len4 && avcc &&
                    avcc.l_size_m_1==3 && sample.type=='video' &&
                    sample.duration<10)
                {
...
MP4ParserStream.prototype.push = function(chunk){
    ...
    if (trigger_on_headers.includes(opt.type))
        this.trigger('header_found', {type: opt.type, pos: this.buffer.b_pos});
    if (this.metadata.h_parsed)
        this.buffer.pos = this.c_parser.parse(opt);
    else
    {
        this.buffer.advance(opt.ptr-opt.offset);
        this.buffer.pos = this.buffer.b_pos+(opt.type=='mdat' ?
            opt.size+opt.offset : this.buffer.b_size);
    }
    return this.buffer.pos;
};
MP4ParserStream.prototype.seek = function(time, use_ssync, opt){
    if (opt && opt.offset!==undefined)
        return void (this.buffer.pos = opt.offset);
    this.trigger('data', {type: 'seek'});
    var seek_info = this.c_parser.seek(time, use_ssync);
    seek_info.prev = this.buffer.pos;
    this.buffer.pos = seek_info.offset;
    return seek_info;
};
MP4ParserStream.prototype.get_buf_pos = function(){ return this.buffer.pos; };
...
AudioFilterStream.prototype.push = function(packet){
    ...
    packet.pts = Math.round(packet.pts*scale);
    packet.dts = Math.round(packet.dts*scale);
    this.trigger('data', {
        type: 'audio',
...
var MP4BuilderStream = function(opt){
    ...
    this.on('confupdate', function(conf){
        ...
        this.options.mark_non_sync = conf.mark_non_sync;
        this.options.disable_edts = conf.disable_edts;
    });
...
MP4BuilderStream.prototype.push = function(packet){
    ...
    if (packet.type=='metadata')
    {
        ...
        return void setTimeout(this.flush_meta.bind(this), 0);
    }
    if (packet.type=='seek')
    {
    ...
    {
        var scale = 90000/packet.ts;
        sample.duration = Math.round(sample.duration*scale);
        sample.pts = Math.round(sample.pts*scale/sample.duration)*
            sample.duration;
        sample.dts = Math.round(sample.dts*scale/sample.duration)*
            sample.duration;
        sample.flags = {
        ...
        if (!this.options.break_on_count || this.options.mark_non_sync)
...
MP4BuilderStream.prototype.flush_meta = function(){
    if (this.inited)
        return;
    this.inited = true;
    var inits = [], _this = this;
    this.metadata.tracks.forEach(function(tr){
        ...
        case 'video':
            ...
            if ((avcc = tr.dr.avcc))
            {
                tr.profileIdc = avcc.avc_p_i;
                tr.levelIdc = avcc.avc_l_i;
                tr.profileCompatibility = avcc.prof_compat;
                tr.sps = avcc.sps.map(function(e){ return e.nal; });
                tr.pps = avcc.pps.map(function(e){ return e.nal; });
            }
        ...
        if (tr.edit_list)
        {
            tr.edit_list.forEach(function(el){ el.segment_duration = 0; });
            if (_this.options.disable_edts)
                delete tr.edit_list;
        }
        inits.push({
            id: tr.id,
