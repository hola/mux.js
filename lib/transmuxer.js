...
AudioSegmentStream = function(track, options) {
  ...
  this.flush = function() {
    ...
    moof = mp4.moof(sequenceNumber, [track], this.options);
...
CoalesceStream.prototype.flush = function() {
  ...
  if (!this.inited || !this.options.no_multi_init)
  {
    this.options.set_duration = 1;
    if (this.options.no_combine)
    {
        var _this = this;
        initSegment = [];
        this.pendingTracks.forEach(function(tr){
            initSegment.push({
                id: tr.id,
  ...
  for (i = 0; i < this.pendingBoxes.length; i++) {
    if (this.options.no_combine)
    {
        event.data = new Uint8Array(this.pendingBoxes[i]);
        event.id = this.pendingTracks[i].id;
        this.trigger('data', event);
        event = {}; // cleanup
        continue;
    }
  ...
  if (this.options.input_type=='m2ts')
  {
    event.captions = [];
    event.metadata = [];
...
Transmuxer = function(options) {
  var
    ...
    packetStream, parseStream, elementaryStream, mp4parseStream,
    aacStream, h264Stream, videoFilterStream, audioFilterStream,
  ...
  {
    videoFilterStream = new muxjs.VideoFilterStream(options);
    audioFilterStream = new muxjs.AudioFilterStream(options);
    elementaryStream
      .pipe(videoFilterStream);
    elementaryStream
      .pipe(audioFilterStream);
  ...
  elementaryStream.on('data', function(data) {
    ...
    if (data.type === 'metadata') {
      ...
      if (videoTrack && !videoSegmentStream) {
        ...
        videoSegmentStream = new VideoSegmentStream(videoTrack, options);
      ...
      if (audioTrack && !audioSegmentStream) {
        ...
        coalesceStream.numberOfTracks++;
        audioSegmentStream = new AudioSegmentStream(audioTrack, options);
